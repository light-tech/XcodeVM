...
struct options {
    bool read_from_stdin;    /* True if data should be read from read_from_stdin. */
...
static void usage(char const* error) {
    ...
    fprintf(stderr,
        ...
        "https://github.com/light-tech/XcodeVM\n"
        "\n"
...
static void parse_args(int* argc, char const** argv, struct options* opts) {
    for (int i = 0; i < *argc; ++i) {
        ...
        if      (strcmp(argv[i], "-")  == 0) opts->read_from_stdin = true;
...
static inline size_t cpio_out(char *buffer, size_t size) {
    size_t c = 0;
    while (c < size) {
        c+= fwrite(buffer + c, 1, size - c, output_file);
    }
    return c;
}
...
int main(int argc, const char** argv) {
    ...
    if (!opts.read_from_stdin && argc < 2)
        usage("missing filename argument");
    else if ((!opts.read_from_stdin && argc > 2) || (opts.read_from_stdin && argc > 1))
    ...
    char xbuf[XBSZ];
    char* zbuf = (char*)malloc(ZBSZ);

    output_file = fopen("Content.cpio", "wb");
    if (output_file == NULL) {
    ...
    free(zbuf);
    lzma_end(&zs);
    if (!opts.read_from_stdin) stream_close(&stream);
    return 0;
}
